...
export default function OutboundLeases() {
  ...
  const filteredData = leases.filter((lease) => {
    if (searchText) {
      ...
      const matchesInn = lease.counterparty?.inn?.toLowerCase().includes(q);
      const matchesAsset = lease.asset?.name?.toLowerCase().includes(q);
      const matchesCounterparty = lease.counterparty?.name?.toLowerCase().includes(q);
      if (!matchesInn && !matchesAsset && !matchesCounterparty) return false;
    }
    if (statusFilter && lease.status !== statusFilter) return false;
    if (categoryFilter && lease.asset?.category !== categoryFilter) return false;
    return true;
  });
  ...
  const columns: ColumnDef<LeaseRecord>[] = [
    ...
    { id: 'assetName', header: 'Asset Name', accessorFn: (row) => row.asset?.name ?? '—' },
    {
    ...
    { id: 'counterparty', header: 'Counterparty', accessorFn: (row) => row.counterparty?.name ?? '—' },
    { id: 'inn', header: 'INN', accessorFn: (row) => row.counterparty?.inn ?? '—' },
    {
  ...
  const handleFormSubmit = (data: any) => {
    if (editingLease) updateMutation.mutate({ id: editingLease.id, data });
    else createMutation.mutate(data);
  };

  const exportCols = [
    { header: 'Asset', accessor: 'asset.name' },
    { header: 'Status', accessor: 'status' },
    { header: 'Counterparty', accessor: 'counterparty.name' },
    { header: 'INN', accessor: 'counterparty.inn' },
    ...
    <div id="outbound-page" className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-sqb-grey-secondary">Management of bank-owned assets for external rent.</p>
        </div>

        <div id="action-bar" className="flex flex-wrap gap-2">
          <button
            className="sqb-btn-primary bg-sqb-navy flex items-center gap-2"
            onClick={() => { setEditingLease(null); setIsFormOpen(true); }}
          >
            <Plus size={16} /> {t('actions.add')}
          </button>
          <button className="sqb-btn-ghost flex items-center gap-2" disabled={selectedLease?.status !== 'INTRODUCED'} onClick={() => { if (selectedLease) { setEditingLease(selectedLease); setIsFormOpen(true); } }}>
            <Edit2 size={16} /> {t('actions.edit')}
          </button>
      ...
      <div id="table-controls" className="flex items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" placeholder="Search by INN or Asset Name..." value={searchText} onChange={(e) => setSearchText(e.target.value)} className="w-full bg-sqb-bg border-none rounded-lg py-2 pl-10 pr-4 text-sm focus:ring-2 focus:ring-sqb-navy/20 outline-none transition-all" />
        </div>
        <div className="flex gap-2">
          <select className="bg-sqb-bg border-none rounded-lg px-3 py-2 text-xs font-bold text-sqb-navy outline-none" value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}>
            <option value="">All Categories</option>
            <option value="PC">PC</option><option value="FURNITURE">Furniture</option><option value="BUILDING">Building</option><option value="LAND">Land</option><option value="ATM">ATM</option>
          </select>
          <select className="bg-sqb-bg border-none rounded-lg px-3 py-2 text-xs font-bold text-sqb-navy outline-none" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
            ...
            <option value="INTRODUCED">Introduced</option><option value="APPROVED">Approved</option><option value="RETURNED">Returned</option>
          </select>
        </div>
      </div>
      ...
      <LeaseFormModal isOpen={isFormOpen} onClose={() => { setIsFormOpen(false); setEditingLease(null); }} onSubmit={handleFormSubmit} direction="OUTBOUND" editData={editingLease} />
      <ProtocolModal isOpen={isProtocolOpen} onClose={() => setIsProtocolOpen(false)} entityId={selectedRow || ''} entityName={selectedLease?.asset?.name} />
    </div>
  );
}
